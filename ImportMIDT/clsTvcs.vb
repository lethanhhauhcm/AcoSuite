...
Public Class clsTvcs
    ...
    Public Function ConnectMidtDatabaseWeb() As Boolean
        ...
        mstrConnectionString = "Data Source=118.69.81.103;Initial Catalog=Mktg_MIDT;UID=user_midt;Pwd=VietHealthy@170172#"
    ...
    Public Function GetRoe(ByVal strCur As String) As Decimal
        ...
        strQuerry = "Select top 1 BSR from DATA1A_ForEX where IsActive='Y'"
    ...
    Public Function CheckDupTkts(ByVal strTkno As String, ByVal strSRV As String) As Boolean
        ...
        If Len(strTkno) < 15 Then strTkno = FormatRasTkno(strTkno)

        strQuerry = "select TKNO from tkt_1a where TKNO ='" & strTkno & "' and SRV='" _
                    & strSRV & "'"
        cmdSql.CommandText = strQuerry
    ...
    Public Function GetCorpName(ByVal strCmc As String) As String
        Dim strQry As String
        Dim cmdSql As New SqlClient.SqlCommand
        cmdSql.Connection = pobjSql.Connection

        strQry = "select CompanyName from GO_CompanyInfo"
    ...
    End Function
    Public Function DuplicateTkt_1A(ByVal mstrTKNO As String, ByVal mstrSRV As String) As Boolean
        ...
        If Len(mstrTKNO) < 15 Then
            mstrTKNO = FormatRasTkno(mstrTKNO)
        End If

        strQuerry = "select * from tkt_1a where TKNO ='" & mstrTKNO & "' and SRV='" _
                    & mstrSRV & "'"
        cmdSql.CommandText = strQuerry
        sglResult = cmdSql.ExecuteScalar
        If sglResult = 0 Then
            DuplicateTkt_1A = False
        Else
            DuplicateTkt_1A = True
        End If
    End Function
    ...
    Public Function CheckFormatRtgType(ByVal strRtgType As String) As Boolean
        ...
        End If
    End Function
    Public Function CheckFormatRtg(ByVal strRtg As String) As Boolean

        If strRtg = "" Then
            CheckFormatRtg = True
        ElseIf ((Len(strRtg) - 3) Mod 5) <> 0 Then
            Return False
        Else
            ...
            intCarCount = (Len(strRtg) - 3) / 5
            For i = 1 To intCarCount
                strCities = strCities & Mid(strRtg, (i - 1) * 5 + 1, 3) & ","
                strCarriers = strCarriers & Mid(strRtg, (i - 1) * 5 + 4, 2) & ","
            Next
            strCities = RemoveLastChr(strCities)
            strCarriers = RemoveLastChr(strCarriers)
            If CitiesExist(strCities) And CarriersExist(strCarriers) Then
                CheckFormatRtg = True
            Else
                CheckFormatRtg = False
            End If
        End If
    ...
    Public Function IsIncentiveCalculated(dteFromDate As Date, dteToDate As Date) As Boolean
        If GetScalarAsString("Select top 1 RecId from Data1A_IncentiveSumBkg where ValidDate between '" _
                             & CreateFromDate(dteFromDate) & "' and '" & CreateToDate(dteToDate) & "'") = "" Then
            Return False
        Else
            Return True
        End If
    ...
    Public Function CreateBalanceQuerries(intChargeId As Integer, decDeductionAmt As Decimal _
                                          , strShortName As String) As List(Of String)
        Dim lstQuerries As New List(Of String)

        If decDeductionAmt <=
            GetScalarAsDecimal("Select isnull(sum(VND),0) from Data1a_Balance" _
            ...
            lstQuerries.Add("Update [Data1A_ProductCharges] set Status='RR'" _
                        ...
                        & "',LstUpdate=getdate() where RecId=" & intChargeId)

            lstQuerries.Add("insert into DATA1A_Balance (ShortName, VND, FOP, TrxDate" _
                            & ", Description, Status, FstUser,ChargeId) values ('" & strShortName _
                            & "', -" & decDeductionAmt _
                            & ",'---',getdate()" _
    ...
    '^_^20220815 add by 7643 -b-
    Public Function LoadListbox(ByRef lstInput As ListBox, ByVal strQuerry As String) As ListBox
        Dim daConditions As SqlClient.SqlDataAdapter
        Dim dsConditions As New DataSet
        ...
        lstInput.DisplayMember = "Value"
        lstInput.ValueMember = "Value"
        LoadListbox = lstInput
        dsConditions.Dispose()
        daConditions.Dispose()
